...
layout = html.Div(children=[
    ...
    dbc.Row(children=[
        dbc.Col([
            ...
            # Top 7 Values
            html.Div(dcc.Dropdown(id='top-input',
        ...
        dbc.Col([
            ...
            html.Div(dcc.Dropdown(id='category-input',
                                  options=[{'label': i, 'value': i}
                                           for i in ['Apples', 'Apricots', 'Bananas', 'Berries', 'Cantaloupe',
                                                     'Cherries', 'Clementines', 'Dates', 'Figs', 'Fruit cocktail',
                                                     'Grapefruit', 'Grapes', 'Honeydew', 'Kiwi', 'Mangoes',
                                                     'Nectarines', 'Oranges', 'Papaya', 'Peaches', 'Pears',
                                                     'Pineapple', 'Plum', 'Pomegranate', 'Watermelon']
                                           ],
                                  value='Apples'
                                  ))
        ],
...
@callback(
    ...
    [
        ...
        Input(component_id='top-input', component_property='value')
    ])
def fruits_categories_prices(category_input, top_input):
    ...
    if top_input == 'All':
        ...
        return bar_chart(data=df, x_column='RetailPrice', y_column='Fruit Category', x_label='Retail Price (Per Pound)',
                         y_label='Fruit Category', title='Average Fruit Prices by Fruit Category', color=all_values)
    elif top_input == 'Top 7':
        df = fruit_prices.groupby('Fruit Category')[['RetailPrice']].mean().reset_index().sort_values(
            by='RetailPrice', ascending=False)[:7]
        df = df.sort_values(by='RetailPrice')
        return bar_chart(data=df, x_column='RetailPrice', y_column='Fruit Category', x_label='Retail Price (Per Pound)',
                         y_label='Fruit Category', title='Average Fruit Prices by Fruit Category', color=top)
    elif top_input == 'Least 7':
        df = fruit_prices.groupby('Fruit Category')[['RetailPrice']].mean().reset_index().sort_values(
            by='RetailPrice')[:7]
        df = df.sort_values(by='RetailPrice', ascending=False)
        return bar_chart(data=df, x_column='RetailPrice', y_column='Fruit Category', x_label='Retail Price (Per Pound)',
                         y_label='Fruit Category', title='Average Fruit Prices by Fruit Category', color=least)
...
def fruits_prices(category_input, top_input):
    if category_input:
        df = fruit_prices[fruit_prices['Fruit Category'] == category_input].sort_values(by='RetailPrice')
        # Pivot the DataFrame
        df_pivoted = df.pivot_table(index='Fruit', columns='Form', values='RetailPrice', fill_value=0)
        ...
        return stacked_bar_chart(data=df_pivoted, x_column='Fruit', currency='$',
                                 z_column=the_list, x_label='Retail Price (Per Pound)', y_label='',
                                 title='Average Fruit Prices' + ' ' + category_input + ' Categories')
...
def fruits_yield(category_input, top_input):
    if category_input:
        df = fruit_prices[fruit_prices['Fruit Category'] == category_input].sort_values(by='Yield')
        # Pivot the DataFrame
        df_pivoted = df.pivot_table(index='Fruit', columns='Form', values='Yield', fill_value=0)
        ...
        return grouped_bar_chart(data=df_pivoted, x_column='Fruit', currency='%', z_column=the_list,
                                 x_label='Preparation Yield (%)', y_label='',
                                 title='Percentage Fruit Yield' + ' ' + category_input + ' Categories',
                                 formatting=100)
